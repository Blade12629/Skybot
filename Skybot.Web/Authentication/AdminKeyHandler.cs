...
{
    public class AdminKeyHandler : AuthenticationHandler<AdminKeyOptions>
    {
        public IServiceProvider ServiceProvider { get; set; }

        public AdminKeyHandler(IOptionsMonitor<AdminKeyOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock, IServiceProvider serviceProvider)
        ...
        protected override Task<AuthenticateResult> HandleAuthenticateAsync()
        {
            ...
            Permission perm = c.Permission.FirstOrDefault(p => p.DiscordUserId == user.DiscordUserId &&
                                                               (AccessLevel)p.AccessLevel == AccessLevel.Dev);

            if (perm == null)
                return Task.FromResult(AuthenticateResult.Fail("Invalid API Key"));

            Claim[] claims = new Claim[] { new Claim("admintoken", hashedKey) };
